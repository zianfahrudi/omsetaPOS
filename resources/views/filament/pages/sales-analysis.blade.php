<x-filament-panels::page>
    ...
    <x-filament::section>
        <div style="font-size:.875rem;color:var(--fi-color-gray-500,#6b7280);">Total Penjualan Periode</div>
        <div style="font-size:1.5rem;font-weight:800;color:var(--fi-color-success-600,#16a34a);">{{ $this->rupiah($r['total']) }}</div>
    </x-filament::section>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(340px,1fr));gap:1.5rem;">
        ...
        <x-filament::section>
            <x-slot name="heading">Per Pelanggan</x-slot>
            <table style="width:100%;border-collapse:collapse;">
                <thead><tr style="text-align:left;border-bottom:2px solid var(--fi-color-gray-200,#e5e7eb);">
                    <th style="padding:.4rem 0;">Pelanggan</th><th style="padding:.4rem 0;text-align:right;">Total</th>
                </tr></thead>
                <tbody>
                    @forelse ($r['by_customer'] as $row)
                        <tr style="border-bottom:1px solid var(--fi-color-gray-100,#f3f4f6);">
                            <td style="padding:.4rem 0;">{{ $row['label'] }}</td>
                            <td style="padding:.4rem 0;text-align:right;">{{ $this->rupiah($row['total']) }}</td>
                        </tr>
