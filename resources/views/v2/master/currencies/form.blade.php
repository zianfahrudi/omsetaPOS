        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $record->code) }}" class="{{ $input }}" placeholder="IDR" required>
                </div>
                <div>
                    <label class="{{ $lbl }}">Simbol</label>
                    <input type="text" name="symbol" value="{{ old('symbol', $record->symbol) }}" class="{{ $input }}" placeholder="Rp">
                </div>
                ...
                <div>
                    <label class="{{ $lbl }}">Kurs (terhadap default)</label>
                    <input type="number" step="0.000001" min="0" name="exchange_rate" value="{{ old('exchange_rate', $record->exchange_rate ?? 1) }}" class="{{ $input }}" required>
                </div>
                <div class="flex items-end">
                    <label class="inline-flex items-center gap-2 text-sm text-slate-700">
                        <input type="hidden" name="is_default" value="0">
                        <input type="checkbox" name="is_default" value="1" @checked(old('is_default', $record->is_default)) class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                        Mata uang default
                    </label>
                </div>
                <div class="sm:col-span-2">
