    ...
    <form method="POST" action="{{ $action }}" class="max-w-2xl">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div><label class="{{ $lbl }}">Tarif Per Jam (Rp)</label><input type="number" step="0.01" min="0" name="hourly_rate" value="{{ old('hourly_rate', $employee->hourly_rate) }}" class="{{ $input }} text-right" required></div>
