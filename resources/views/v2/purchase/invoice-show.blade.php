...
@section('heading', 'Detail Faktur Pembelian')
...
@section('content')
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <dl class="mt-4 grid grid-cols-1 gap-3 text-sm sm:grid-cols-3">
            <div><dt class="text-slate-400">Pemasok</dt><dd class="text-slate-700">{{ $invoice->supplier?->name ?: '—' }}</dd></div>
            <div><dt class="text-slate-400">Jatuh Tempo</dt><dd class="text-slate-700">{{ $invoice->due_date?->format('d/m/Y') ?: '—' }}</dd></div>
            <div><dt class="text-slate-400">No. Faktur Pemasok</dt><dd class="text-slate-700">{{ $invoice->supplier_invoice_no ?: '—' }}</dd></div>
        </dl>
                ...
                <tbody>
                    @foreach ($invoice->items as $item)
                        <tr class="border-b border-slate-100">
                            ...
                            <td class="py-2.5 text-right">{{ $rp($item->unit_cost) }}</td>
