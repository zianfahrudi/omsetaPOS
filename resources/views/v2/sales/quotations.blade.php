...
@section('heading', 'Penawaran Harga')
...
@section('content')
    <div class="mb-4 flex items-center justify-between gap-3">
        ...
        <a href="{{ route('v2.sales.quotations.create') }}" class="shrink-0 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">+ Penawaran</a>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-200 bg-slate-50 text-left text-slate-500">
                        ...
                        <th class="px-4 py-3 font-medium">Berlaku s/d</th>
                ...
                <tbody>
                    @forelse ($records as $r)
                        <tr class="border-b border-slate-100 hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 text-slate-500">{{ $r->valid_until?->format('d/m/Y') ?: '—' }}</td>
                            <td class="px-4 py-3 text-right">{{ $rp($r->grand_total) }}</td>
                            <td class="px-4 py-3 text-center"><x-v2.status :value="$r->status" /></td>
                            <td class="px-4 py-3 text-right">
                                @if ($r->status !== 'ordered')
                                    <form method="POST" action="{{ route('v2.sales.quotations.convert', $r) }}" onsubmit="return confirm('Konversi penawaran ini menjadi pesanan?')">
                                        @csrf
                    ...
                    @empty
                        <tr><td colspan="7" class="px-4 py-10 text-center text-slate-400">Belum ada penawaran.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
