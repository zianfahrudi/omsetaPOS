        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6">
            ...
            <div class="mt-5 space-y-3">
                @foreach ($modules as $groupKey => $group)
                    @php($groupOn = $enabled[$groupKey] ?? true)
                    <div class="rounded-xl border border-slate-200" x-data="{ on: {{ $groupOn ? 'true' : 'false' }} }">
                        {{-- Header grup: toggle seluruh modul --}}
                        <div class="flex items-center justify-between px-4 py-3">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <input type="checkbox" name="keys[]" value="{{ $groupKey }}" x-model="on" class="hidden">
                        ...
                        {{-- Sub-menu: hanya bisa diatur bila grup aktif --}}
                        <div class="border-t border-slate-100 px-4 py-2" :class="! on && 'opacity-40 pointer-events-none'">
                            @foreach ($group['items'] as $itemKey => $itemLabel)
                                @php($itemOn = $enabled[$itemKey] ?? true)
                                <div class="flex items-center justify-between py-2 pl-3" x-data="{ on: {{ $itemOn ? 'true' : 'false' }} }">
                                    <span class="text-sm text-slate-600">{{ $itemLabel }}</span>
                                    <div class="flex items-center gap-3">
                                        <input type="checkbox" name="keys[]" value="{{ $itemKey }}" x-model="on" class="hidden">
                                        <button type="button" @click="on = !on"
                                                class="relative inline-flex h-5 w-9 shrink-0 items-center rounded-full border transition-colors"
                                                :class="on ? 'bg-indigo-600 border-indigo-600' : 'bg-slate-300 border-slate-400'">
                                            <span class="h-4 w-4 rounded-full bg-white shadow transition-all"
                                                  :style="on ? 'margin-left:1.125rem' : 'margin-left:0.125rem'"></span>
                                        </button>
                                    </div>
                                </div>
                            @endforeach
